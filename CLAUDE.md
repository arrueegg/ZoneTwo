...
**AI weekly debrief**: cached 7 days in `athletes.ai_summary`. `POST /recommendations/weekly-summary?force=true` bypasses cache. Output is structured JSON with `week_summary`, `training_recommendation`, `recovery_insight` sections. The prompt must prioritize interpretation, judgement calls, concrete recommendations, and whether the athlete is on track for saved preparation targets over repeating metric values. The router includes upcoming `training_events`, current preparation phase, saved `planned_workouts`, and completed-vs-planned weekly distance context in the LLM payload. Multi-strategy fallback parser in `services/recommendations.py:_extract_json` handles non-compliant LLM output.
